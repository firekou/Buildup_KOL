...
export const NARRATIVE_SHAPES = {
  ...
  framework: {
    label: '觀念框架型',
    says: '把事件拆成 2–3 個角度，每個角度給一句可帶走的判準，收在一個下次也能用的觀念。讀者的感受是「我學到了」而不是「他在推銷」。',
    beats: (topic) => [
      `承接題目：${topic}`,
      '指出這件事真正在變的是什麼——多數人只看到表面那一層',
      '拆成 2–3 個角度，每個角度給一句讀者可以帶走的判準（用序號列出）',
  ...
  demo: {
    ...
    beats: (topic) => [`承接題目：${topic}`, '示範產品實際被用來解決什麼', '指出沒有它的時候會卡在哪'],
  },
  evidence: {
    label: '攤證據型',
    says: '把可查證的資料攤開，讓讀者自己判斷。適合 database 型人設。',
    beats: (topic) => [`承接題目：${topic}`, '把可查證的數據攤開', '說明這份數據怎麼查、怎麼自己驗一次'],
  },
}
