...
2. **鎖臉（已完成 v1）：**
   - **已建立 Reference Element：** `faye-tan-a02` → **`<<<94a1aa08-c337-4125-b85b-b3eaa7a02463>>>`**（基準臉 job `303f8566-cfdf-40cd-bc0f-f6bbcb0a1bfc`）。之後所有場景 prompt 直接在開頭嵌入此 element_id 即可鎖臉。
...
4. **存檔：** 把 `element_id` 與各 job id 記錄回 `profile.json` 的 `ai_assets`，圖片放進 `kols/faye-tan/images/`。
