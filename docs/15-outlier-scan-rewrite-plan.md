...
**規格：** `docs/14-outlier-scan-spec.md` **v1.1**
...
**狀態：** ✅ **R0（先修規格）已完成** → 🔜 **R1（Batch 0 來源探測）可開工** → ⏸ **Batch 1–5 待 Batch 0 結果**
**建立於：** 2026-08-24（v1.0）／**修訂於：** 2026-08-24（v1.1）
...
| **R1** | Batch 0 擴充成 10 項硬驗收，重點是**半年窗到底抓不抓得到** | 🔜 可開工（§2） |
...
| **0-4** 🆕 | **hashtag / keyword actor 真的能覆蓋 180 天嗎？還是只回 recent / top N？** | **「半年內」這個前提本身**。見上 |
| **0-5** 🆕 | 分頁可控嗎？最多能翻到多舊？ | 同上；也決定成本上限 |
...
**0-4 / 0-5 失敗（抓不到 180 天）：** 那就**不能叫「半年內離峰值掃描」**。要嘛改成 actor 實際能覆蓋的窗長並跟使用者確認，要嘛換資料源。**不得沿用「半年」這個名字去跑一個實際只有 30 天的窗**——那是本 repo 反覆在修的同一類錯（`growth7d` 其實是 48 小時、`volume` 其實是樣本共現次數）。
...
| 風險 | 嚴重度 | 處理 |
|---|---|---|
| **【v1.1 新增】半年窗抓不到** | 🔴 **比抓不到基準更根本** | 批次 0-4 / 0-5 先驗。抓不到就不能叫「半年內離峰值掃描」——沿用這個名字去跑一個 30 天的窗，是本 repo 反覆在修的同一類錯 |
...
2. **【v1.1 新增】批次 0-4 / 0-5 若失敗（抓不到 180 天），窗長改成多少？** 這決定這件事還能不能叫「半年內」。
