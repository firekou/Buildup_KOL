...
**規格：** `docs/14-outlier-scan-spec.md` **v1.3**
...
**狀態：** ✅ **R0 已完成** → ✅ **R1 完成：Batch 0 十項全部跑完**（`docs/reviews/2026-08-25-scan-source-probe.md` ＋ `-part2.md`）→ 🔜 **R2（Batch 1 起）可開工**
**建立於：** 2026-08-24（v1.0）／**修訂於：** 2026-08-24（v1.1）、2026-08-25（v1.2 與 v1.3，Batch 0 十項實測回填）
...
| **R1** | Batch 0 擴充成 10 項硬驗收 | ✅ **十項全部跑完**（§2.1） |
| **R2** | Batch 1 起的實作，且**不順帶新增 `health` scoring domain** | 🔜 **可開工**——0-3 通過，主判準成立 |
...
| # | 要查什麼 | 影響到什麼 |
|---|---|---|
| **0-1** ✅ | ~~各地區參數可用嗎？~~ **已跑。七區全回 200／30–38 則／連打 5 次無限制。但要走 `rss/search` 不是主題 feed——差三十倍觸及** | `newsQueries` 從設定資料升格為必要輸入 |
| **0-2** ⚠️ | ~~Yahoo 路徑是什麼？~~ **已跑。TW／HK 可用（100 則），SG 只 5 則、JP 只 8 則、US 403** | Yahoo 軌只在 TW／HK 成立；而且它是**一家媒體**，不能併進 `outletCount` |
| **0-3** ✅ | ~~能列出帳號近期影片嗎？~~ **已跑。五個帳號全部 `viewsCoverage 1.0`、`staysOnProfile true`、23–30 篇** | **主判準成立。§2.2 的三個備案都不必動用** |
...
| **0-6** ⚠️ | ~~每帳號幾支、跨度多大？~~ **已跑。23–30 篇、跨度 61–297 天** | 基準是「最近 N 篇」不是「最近 180 天」→ 常數改名 `baselineRecentPosts` |
| **0-7** ✅ | ~~上游是什麼排序？~~ **已跑。`medianViewsInSample` 對 `upstreamRank` 無單調趨勢——不是 popularity 排序** | 截斷偏誤比預期輕。帳目仍要記（actor 換版本就可能變） |
| **0-8** ⬜ | ~~私人／已刪除怎麼標？~~ **未觀測**——五個受測帳號都是公開正常的 | 不是通過也不是失敗。要判定得刻意找已知的私人帳號 |
| **0-9** ⚠️ | ~~重跑穩定嗎？~~ **已跑。`recallOfA 0.48`——背對背兩次只有一半重疊** | **這一輪最重的發現。** 跨週比較要打折；單次掃描內的跨頻道重現更可靠 |
| **0-10** ⚠️ | ~~成本與耗時？~~ **已跑。每作者 21.6 秒，100 個 = 36.1 分鐘** | 超過 30 分鐘 → §1.5 的 durable runner 從「該做」變成「非做不可」。計費仍未量 |

**批次 0 的產出是記錄真實回應的探測報告，兩份都已寫好：**
`docs/reviews/2026-08-25-scan-source-probe.md`（0-4／0-5）與 `-part2.md`（其餘八項）。

**【v1.3】十項的總結：0-3 通過，主判準成立，Batch 1 可開工。** 三個最重要的發現：
1. **0-3 通過** — 頻道中位數算得出來，§2.2 的三個備案都不必動用
2. **0-9 的 48% churn** — 一次掃描是一份抽樣不是母體，跨週比較要打折
3. **0-1 的 feed 選擇** — Track B 要走 `rss/search`，主題 feed 只有 0.7 天觸及
...
**~~0-3 失敗~~【v1.3 · 已跑，通過】** 五個帳號全部 `viewsCoverage 1.0`、`staysOnProfile true`、23–30 篇，全部高於 `minBaselineVideos = 8`。**頻道中位數算得出來，主判準成立。** 下面三個備案保留在文件裡作為紀錄，但**都不需要動用**：
...
| `dashboard/server/lib/topics/apify.js` | **部分已完成**：`buildProfileInput()` 與 `toPost` 的 `id` 已在 Batch 0 探測時加上並實測通過。剩下的是把它接進正式的抓取流程（`fetchChannelVideos`） |
...
| ~~抓不到頻道基準~~ **【v1.3 已解除】** | — | 0-3 實測通過，主判準成立 |
| **重跑就換一批樣本【v1.3 · 已實測】** | 🟡 **降級：不推翻，但要打折** | 0-9 實測 `recallOfA 0.48`。設計已用「單次掃描內的跨頻道重現」繞開（`docs/14` §8.2.2），跨週宣稱要保守。**這是平台行為不是 bug，無法消除** |
| RSS 端點與預期不符 | 🟡 Track B 延後 | 批次 0 先驗；Track A 不依賴它，可先上 |
| 抓取成本失控 | 🟡 **已量耗時** | 每作者 21.6 秒、100 個 = 36.1 分鐘。`maxChannelFetches` 已定 100。**計費仍未量** |
...
1. ~~批次 0-3 若失敗，走哪個備案？~~ **【v1.3 · 不需裁示了】** 0-3 通過，主判準成立。
2. ~~批次 0-4 / 0-5 若失敗，窗長改成多少？~~ **【v1.2 · 不需裁示了】** TikTok 撐得住 180 天。但衍生出一個新的：**Track A 只剩 TikTok，這樣夠嗎？** 若要多平台，就回到 0-3 的備案 A（接 YouTube）。
3. **【v1.3 新增】Yahoo 軌只在 TW／HK 成立，其他區只有 Google——可以接受嗎？** SG 只回 5 則、JP 只回 8 則、US 403。
4. **【v1.1】要不要另開一個 `health` scoring domain 的 migration？** 不開的話，健康型方向的 `fit` 會一直比其他方向粗（§3.1）。這件事現在**不擋**掃描開發，但值得排進待辦。
5. **政治型方向要不要做人設？** 目前 16 位沒有任何一位落在這裡。不做的話，這個方向的掃描永遠只能是題目盤點。
6. **`mia-huang` 的直播定位。** `docs/12` §3.1 已判定 AI 人設在結構上無法履行即時雙向的直播。遊戲型方向的掃描結果送到她身上之前，這件事要先解決。
